<?php
	...
	if($_GET)
	{
		if(isset($_GET['query']))
		{
			$Resultados = getServidoresByIdUtilizadorAndQuery($User->getId(), $_GET['query']);
		}
		else
		{
	...
	$smarty->assign("Zona", $linguagens[$User->getLinguagem()]['zona_procurar']." :: ".$_GET['query'], true);
	...
	$smarty->assign("ServidoresProcura", $Resultados);
	$smarty->assign("NumServidoresProcura", count($Resultados));
	$smarty->assign("Query", htmlentities($_GET['query']));
	...
	$smarty->display($User->getPastaTema().'/procura.tpl');
...
<script type="text/javascript">
  ...
  <?php
    foreach($Resultados as $Servidor)
    {
      $server = new Servidor($Servidor['Id'], $conn);
  ?>
